...
function addRow() {
  ...
  cell2.innerHTML = `${value2} W`;
  cell3.innerHTML = value3;
  cell4.innerHTML = '<button class="btn btn-danger" onclick="deleteRow(this)">Usuń</button>';
}
...
function get_data_from_table() {
  ...
  // Display the extracted data (you can use it as needed)
  console.log(data);
...
function displayPlotConsumption(plot_data, id, cumulative=0) {
  ...
  var consumption = {
    ...
    stackgroup: 'one',
    hovertemplate: (cumulative) ? 'Data: %{x}<br>Skumulowane zużycie: %{y} MWh<extra></extra>' : 'Data: %{x}<br>Zużycie: %{y} kWh<extra></extra>',
    yaxis: 'y2',
  };
  ...
  var layout = {
    ...
    yaxis2: {
      title: (cumulative) ? 'Skumulowane zużycie (MWh)' : 'Zużycie (kWh)',
      titlefont: {
        color: '#31D843',
        size: 21,
        weight: 'bold',
...
document.getElementById("button-calculate").onclick = function() {
  if (checkParametersInput()) {
    ...
    .then(response_data => {
        ...
        displayPlotConsumption(plot_data=response_data, id='plot-consumption');
        displayPlot(plot_data=response_data, id='plot');
        displayPlot(plot_data=response_data, id='plot-cumulative', cumulative=1);
        // displayPlot2(plot_data=response_data, id='plot-cumulative', cumulative=1);
